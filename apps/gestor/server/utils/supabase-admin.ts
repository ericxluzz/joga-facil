...
export function mapScheduleRule(row: Record<string, any>) {
  return {
    id: row.id as string,
    tenantId: row.tenant_id as string,
    resourceId: row.resource_id as string,
    ...
    priceModifier: parseFloat(row.price_modifier as string),
    active: row.active as boolean,
  };
}

export function mapBlock(row: Record<string, any>) {
  return {
    id: row.id as string,
    tenantId: row.tenant_id as string,
    resourceId: (row.resource_id as string | null) ?? null,
